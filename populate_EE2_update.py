...
def populate(ee_science_ii):
    ...
    add_theory_element(theory=digilent_board_theory, name="digilent theory 1", number=1,
					   ...
					   "Also, here we shall introduce the formula for charging and discharging current formula for a series RC circuit. Consider Fig.1.",
    ...
    add_theory_element(theory=digilent_board_theory, name="digilent theory 4", number=4,
                       text_input="This circuit has a switch. When the switch is in position '1', the capacitor is being charged "+
                       "(assuming zero initial charge). And after that, if the switch is changed to the position '2', the capacitor is being discharged. "+
                       "For position '1' of switch, the charging current that flows through the resistor experiences an exponential decay. The charging "+
    ...
    add_theory_test_question(theorytest=digilent_board_theory_test,
                             question="Figure numbers are specific to the test. Refer to Fig. 1 what would be the initial voltage of the capacitor when the switch is just closed?",
    ...
    add_theory_test_question(theorytest=digilent_board_theory_test,
                             question="Refer to the circuit in Fig. 1. At t=0.6 ms, what would be the capacitor voltage?",
    ...
    add_theory_test_question(theorytest=digilent_board_theory_test,
                             question="Refer to the circuit in Fig. 2. Given, Vs = 6V, R = 5ohms, C = 10 uF. Suppose, after a long time, if the charging battery is removed and the capacitor "+
    ...
    add_theory_test_question(theorytest=digilent_board_theory_test,
                             question="Refer to the circuit in Fig. 2. Given, Vs = 6V, R = 5ohms, C = 10 uF. What will be the charging current at the instant t = 50 microseconds? Use the charging current formula.",
    ...
    add_theory_test_question(theorytest=digilent_board_theory_test,
                             question="Refer to the circuit in Fig. 2. Assume that initial capacitor voltage is zero. When the capacitor in the RC circuit above reaches its maximum voltage, which of the following statements is FALSE? ",
    ...
    add_simulation_element(simulation=digilent_board_simulation,
                           ...
                           "\(R=1k\\Omega\), \(C=0.1\\mu F\). Keep the simulation circuit for comparing with hardware results later.",
                           image_input=None, equation_input="", video_input="", 
                           element_type='text')
    add_simulation_element(simulation=digilent_board_simulation,
    ...
    # Digilent Board Intro using RC circuit simulation test
    digilent_board_simulation_test = add_simulation_test(lab=digilent_board_intro, name="Digilent Board Intro Simulation Test")

    add_sim_test_element(simulationtest=digilent_board_simulation_test, name="digilent simulation test element 1", number=1,
                      text_input="",
                      image_input='VLA/courses/EE_Science_II/Lab02/fig05.jpg', equation_input="", video_input="",
                      element_type='image')

    add_sim_test_element(simulationtest=digilent_board_simulation_test, name="digilent simulation test element 2", number=2,
                      text_input="Figure 1",
                      image_input=None, equation_input="", video_input="", 
                      element_type='caption')

    
    add_simulation_test_question(simulationtest=digilent_board_simulation_test,
                                 question="Figure number is specific to this test. Refer to the circuit in Fig. 1. with \(V_s=10V\),\(R=1k\\Omega\), \(C=1\\mu F\) "+
    ...
    add_simulation_test_question(simulationtest=digilent_board_simulation_test,
                                 ...
                                 answer_three="\(V_o=1 V\)", answer_four="\(V_o=10 V\)",
                                 correct_answer_number=4,
    ...
    add_simulation_test_question(simulationtest=digilent_board_simulation_test,
                                 question="For the circuit in Fig. 1, suppose we change our mind and apply 2V pulse input. What would be the status of the time constant of the RC circuit?",
    ...
    add_hardware_element(hardware=digilent_board_hardware, name="digilent board hardware 2", number=2,
                         ...
                         "on your oscilloscope to help you measure exactly. You shall do the same with the second simulation circuit. However, before that go to the Results section for further instructions.",
                         image_input=None, equation_input="", video_input="", 
                         element_type='text')
    ...
    add_results_questions(results=digilent_board_results,
                          question="Write a standard report of 3 to 6 pages (see syllabus for details). "+
    ...
    add_lab_test_element(labtest=digilent_board_lab_test, name="digilent lab lab element 1", number=1,
                      text_input="",
                      image_input='VLA/courses/EE_Science_II/Lab02/fig03.jpg', equation_input="", video_input="",
                      element_type='image')
    ...
    add_lab_test_element(labtest=digilent_board_lab_test, name="digilent lab lab element 3", number=3,
                      text_input="",
                      image_input='VLA/courses/EE_Science_II/Lab02/fig04.jpg', equation_input="", video_input="",
                      element_type='image')
